...
static void on_send_data(const uint8_t *data, size_t data_length, void *user_data)
{
...
int main(int argc, char *argv[])
{
	...
	for (i = 0; i < 2; i++)
	{
		...
		if (res)
		{
			fprintf(stderr, "secure_session_init: %d\n", res);
			return res;
		}
	}
	...
	if (res)
	{
		fprintf(stderr, "secure_session_connect: %d\n", res);
		return res;
	}

	/* Data is waiting for the server */
	res = secure_session_receive(&(clients[1].session), dummy, sizeof(dummy));
	if (res)
	{
		fprintf(stderr, "secure_session_receive: %d\n", res);
		return res;
	}

	/* Data is waiting for the client */
...
err:
	...
	for (i = 0; i < 2; i++)
	{
		...
		if (res)
		{
			fprintf(stderr, "secure_session_cleanup: %d\n", res);
		}
	}
