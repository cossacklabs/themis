...
static void test_basic_encryption_flow(void)
{
	...
	if (NULL == import_ctx)
	{
		sput_fail_if(NULL == import_ctx, "asym_cipher_ctx != NULL");
		return;
	}
	...
	if (HERMES_SUCCESS != res)
	{
		sput_fail_unless(HERMES_SUCCESS == res, "soter_asym_cipher_export_key fail");
		soter_asym_cipher_destroy(ctx);
		soter_asym_cipher_destroy(import_ctx);
		return;
	}
	...
	if (HERMES_SUCCESS != res)
	{
		sput_fail_unless(HERMES_SUCCESS == res, "soter_asym_cipher_import_key fail");
		soter_asym_cipher_destroy(ctx);
		soter_asym_cipher_destroy(import_ctx);
		return;
	}
	...
	sput_fail_unless(encrypted_data_length_import_ctx == encrypted_data_length, "RSA OAEP encryption with imported key");
	...
	sput_fail_unless((sizeof(test_data) == decrypted_data_length_import_ctx) && !(memcmp(test_data, decrypted_data_import_ctx, sizeof(test_data))), "RSA OAEP decryption with imported key");
