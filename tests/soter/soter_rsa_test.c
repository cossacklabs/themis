...
static void test_basic_encryption_flow(void)
{
	...
	ctx = soter_asym_create(SOTER_ASYM_RSA, SOTER_ASYM_OAEP);
	if (NULL == ctx)
	{
		sput_fail_if(NULL == ctx, "asym_ctx != NULL");
		return;
	}

	res = soter_asym_gen_key(ctx);
	if (HERMES_SUCCESS != res)
	{
		sput_fail_unless(HERMES_SUCCESS == res, "soter_asym_gen_key fail");
		soter_asym_destroy(ctx);
		return;
	}

	res = soter_asym_encrypt(ctx, test_data, sizeof(test_data), encrypted_data, &encrypted_data_length);
	if (HERMES_SUCCESS != res)
	{
		sput_fail_unless(HERMES_SUCCESS == res, "soter_asym_encrypt fail");
		soter_asym_destroy(ctx);
		return;
	}
	...
	res = soter_asym_decrypt(ctx, encrypted_data, encrypted_data_length, decrypted_data, &decrypted_data_length);
	if (HERMES_SUCCESS != res)
	{
		sput_fail_unless(HERMES_SUCCESS == res, "soter_asym_decrypt fail");
		soter_asym_destroy(ctx);
		return;
	}
	...
	res = soter_asym_destroy(ctx);
	if (HERMES_SUCCESS != res)
	{
		sput_fail_unless(HERMES_SUCCESS == res, "soter_asym_destroy fail");
		return;
	}
...
void run_soter_rsa_tests(void)
{
	...
	sput_enter_suite("soter rsa: basic flow");
