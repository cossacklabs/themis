...
class TestSCell(unittest.TestCase):
    def setUp(self):
	self.key="This is test key"
	self.message="This is test message"
	self.context="This is test context"
	...
	with self.assertRaises(themis_exception):
	    decrypted_message=enc.decrypt(encrypted_message+"11")
	...
	with self.assertRaises(themis_exception):
	    decrypted_message=enc.decrypt(encrypted_message+"11", self.context)
	with self.assertRaises(themis_exception):
	    decrypted_message=enc.decrypt(encrypted_message)
	with self.assertRaises(themis_exception):
	    decrypted_message=enc.decrypt(encrypted_message, None)
	with self.assertRaises(themis_exception):
	    decrypted_message=enc.decrypt(encrypted_message, self.context+"11")
	...
	with self.assertRaises(themis_exception):
	    decrypted_message=enc.decrypt(encrypted_message+"11", token)
	with self.assertRaises(TypeError):
	    decrypted_message=enc.decrypt(encrypted_message, None)
	...
	with self.assertRaises(themis_exception):
	    decrypted_message=enc.decrypt(encrypted_message+"11", token, self.context)
	with self.assertRaises(themis_exception):
	    decrypted_message=enc.decrypt(encrypted_message, token)
	with self.assertRaises(TypeError):
	    decrypted_message=enc.decrypt(encrypted_message, None, self.context)
	with self.assertRaises(themis_exception):
	    decrypted_message=enc.decrypt(encrypted_message, token, self.context+"11")
