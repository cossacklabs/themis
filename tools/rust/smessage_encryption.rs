...
fn main() {
    let matches = clap_app!(smessage_encryption =>
        ...
        (@arg secret_key: +required "Path to file with recipient's secret key")
    ...
    let secret_key_path = matches.value_of("secret_key").unwrap();
    let public_key_path = matches.value_of("public_key").unwrap();
    let message = matches.value_of("message").unwrap();

    let secret_key = read_file(&secret_key_path).expect("read secret key");
    ...
    match command {
        ...
        "sign" => {
            let signer = SecureSign::new(secret_key);
