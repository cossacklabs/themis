...
encrypter=smessage.themis_smessage_encrypter(server_priv, client_pub);
decrypter=smessage.themis_smessage_decrypter(server_priv, client_pub);
...
def on_request(ch, method, props, body):
    message = decrypter.decrypt(body);
    print message;
    ch.basic_publish(exchange='', routing_key=props.reply_to, properties=pika.BasicProperties(correlation_id = props.correlation_id), body=encrypter.encrypt(message));
