...
class transport(ssession.mem_transport):
    def get_pub_key_by_id(self, user_id):        #necessary callback function
        if user_id != b"bdKBEanUfQRgwmc":                        #we have only one peer with id "server"
            raise Exception("no such id");
        return base64.b64decode("VUVDMgAAAC0JPqWZA3Q39x/usK0YGphEx5PHclnJ1cY9X7v/OIV0l5pTmrpz"); 

session=ssession.ssession(b"ckLNYmsOxvbxYGy",base64.b64decode("UkVDMgAAAC1whm6SAJ7vIP18Kq5QXgLd413DMjnb6Z5jAeiRgUeekMqMC0+x"), transport())
...
conn.request("POST", "/api/ckLNYmsOxvbxYGy/", params, headers)
