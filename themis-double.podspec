Pod::Spec.new do |s|
    ...
    s.subspec 'openssl' do |so|
        ...
        so.subspec 'objcwrapper' do |ss|
            ...
            ss.dependency 'themis/openssl/core'
        end
    end


    s.subspec 'boringssl' do |so|

        so.dependency 'BoringSSL', '~> 10.0'
        so.ios.deployment_target = '11.0'
        so.osx.deployment_target = '10.10'

        so.ios.xcconfig = { 'OTHER_CFLAGS' => '-DBORINGSSL', 'USE_HEADERMAP' => 'NO', 
        'HEADER_SEARCH_PATHS' => '"${PODS_ROOT}/themis/src" "${PODS_ROOT}/themis/src/wrappers/themis/Obj-C"', 'CLANG_ALLOW_NON_MODULAR_INCLUDES_IN_FRAMEWORK_MODULES' => 'YES' }
        
        so.osx.xcconfig = { 'OTHER_CFLAGS' => '-DBORINGSSL', 'USE_HEADERMAP' => 'NO', 
            'HEADER_SEARCH_PATHS' => '"${PODS_ROOT}/themis/src" "${PODS_ROOT}/themis/src/wrappers/themis/Obj-C"', 'CLANG_ALLOW_NON_MODULAR_INCLUDES_IN_FRAMEWORK_MODULES' => 'YES' }
