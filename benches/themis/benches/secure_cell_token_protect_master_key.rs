...
use libthemis_sys::{
    themis_secure_cell_decrypt_token_protect, themis_secure_cell_encrypt_token_protect,
...
pub fn encryption(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("Secure Cell encryption - Token Protect, master key");
    for message_size in MESSAGE_SIZES {
        ...
        group.bench_with_input(
            ...
            |b, &size| {
                ...
                let mut token = vec![0; MIN_AUTH_TOKEN];
                ...
                b.iter(|| {
                    ...
                    assert!(token_size <= MIN_AUTH_TOKEN);
...
pub fn decryption(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("Secure Cell decryption - Token Protect, master key");
    for message_size in MESSAGE_SIZES {
        group.throughput(Throughput::Bytes(*message_size as u64));
        group.bench_with_input(
            BenchmarkId::from_parameter(pretty(*message_size)),
            ...
            |b, &size| {
                let message = vec![0; size];
                let (encrypted, token) = SecureCell::with_key(&master_key)
                    .expect("invalid key")
                    .token_protect()
