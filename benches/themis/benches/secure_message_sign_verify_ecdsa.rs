...
use themis::{keygen::gen_ec_key_pair, secure_message::SecureSign};
...
pub fn signing(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("Secure Message signing - ECDSA");
...
pub fn verification(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("Secure Message verification - ECDSA");
    for message_size in MESSAGE_SIZES {
        group.throughput(Throughput::Bytes(*message_size as u64));
        group.bench_with_input(
            BenchmarkId::from_parameter(pretty(*message_size)),
