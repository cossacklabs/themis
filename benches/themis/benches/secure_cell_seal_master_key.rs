...
use libthemis_sys::{
    themis_secure_cell_decrypt_seal, themis_secure_cell_encrypt_seal, THEMIS_BUFFER_TOO_SMALL,
    THEMIS_SUCCESS,
};
...
pub fn encryption(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("Secure Cell encryption - Seal, master key");
...
pub fn decryption(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("Secure Cell decryption - Seal, master key");
    for message_size in MESSAGE_SIZES {
        group.throughput(Throughput::Bytes(*message_size as u64));
        group.bench_with_input(
            BenchmarkId::from_parameter(pretty(*message_size)),
