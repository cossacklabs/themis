...
themis_status_t themis_sym_plain_encrypt(uint32_t alg,
  ...
  HERMES_STATUS_CHECK_FREE(soter_sym_encrypt_update(ctx, message, message_length, encrypted_message, encrypted_message_length),HERMES_SUCCESS, ctx);
...
themis_status_t themis_sym_plain_decrypt(uint32_t alg,
  ...
  HERMES_STATUS_CHECK_FREE(soter_sym_decrypt_update(ctx, encrypted_message, encrypted_message_length, message, message_length),HERMES_SUCCESS, ctx);
...
themis_status_t themis_sym_decrypt_message_u_(const uint8_t* key,
					   ...
					   uint8_t* message,
					   size_t* message_length){
  HERMES_CHECK_PARAM(encrypted_message!=NULL && encrypted_message_length!=0);
  HERMES_CHECK_PARAM(context!=NULL && context_length!=0);
