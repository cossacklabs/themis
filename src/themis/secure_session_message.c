...
themis_status_t secure_session_wrap(secure_session_t* session_ctx, const void* message,
                                    size_t message_length, void* wrapped_message,
                                    size_t* wrapped_message_length)
{
    ...
    *length = htonl(message_length + sizeof(uint32_t) + sizeof(uint64_t));
    ...
    res = encrypt_gcm(
        session_ctx->out_cipher_key, sizeof(session_ctx->out_cipher_key), iv, CIPHER_MAX_BLOCK_SIZE,
        length, message_length + sizeof(uint32_t) + sizeof(uint64_t) + sizeof(uint32_t), length,
...
themis_status_t secure_session_unwrap(secure_session_t* session_ctx, const void* wrapped_message,
                                      size_t wrapped_message_length, void* message,
                                      size_t* message_length)
{
    ...
    if (!secure_session_is_established(session_ctx)) {
        res = session_ctx->state_handler(session_ctx, wrapped_message, wrapped_message_length,
    ...
    sym_ctx = soter_sym_aead_decrypt_create(
        SOTER_SYM_AES_GCM | SOTER_SYM_256_KEY_LENGTH, session_ctx->in_cipher_key,
        sizeof(session_ctx->in_cipher_key), NULL, 0, iv, CIPHER_MAX_BLOCK_SIZE);
    if (NULL == sym_ctx) {
        return THEMIS_FAIL;
    }
    ...
    res = soter_sym_aead_decrypt_update(sym_ctx, iv + CIPHER_MAX_BLOCK_SIZE, sizeof(message_header),
    ...
    *message_length = length - (sizeof(uint32_t) + sizeof(uint64_t));
    ...
    /*{
            ...
            {
                    ((uint8_t *)message)[i] = iv[CIPHER_MAX_BLOCK_SIZE + sizeof(message_header) + i]
    ^ 0xff;
            }
    }*/

    res =
        soter_sym_aead_decrypt_update(sym_ctx, iv + CIPHER_MAX_BLOCK_SIZE + sizeof(message_header),
                                      *message_length, message, message_length);
    if (THEMIS_SUCCESS != res) {
        goto err;
    }

    res = soter_sym_aead_decrypt_final(
        sym_ctx, iv + CIPHER_MAX_BLOCK_SIZE + sizeof(message_header) + *message_length,
        CIPHER_AUTH_TAG_SIZE);
    if (THEMIS_SUCCESS != res) {
        goto err;
    }
