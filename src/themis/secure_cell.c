...
themis_status_t themis_secure_cell_encrypt_full(const uint8_t* master_key,
  ...
  size_t ctx_length_;
  size_t msg_length_;
  HERMES_CHECK(themis_auth_sym_encrypt_message(master_key, master_key_length, message, message_length, NULL, 0, NULL, &ctx_length_, NULL, &msg_length_)==HERMES_BUFFER_TOO_SMALL);
  if((*encrypted_message_length)<(ctx_length_+msg_length_)){
...
themis_status_t themis_secure_cell_decrypt_full(const uint8_t* master_key,
  ...
  size_t ctx_length_;
  size_t msg_length_;
  HERMES_CHECK(themis_auth_sym_decrypt_message(master_key, master_key_length, encrypted_message, encrypted_message_length, NULL, 0, NULL, &msg_length_)==HERMES_BUFFER_TOO_SMALL);
