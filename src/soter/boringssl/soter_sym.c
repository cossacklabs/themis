...
soter_status_t soter_pbkdf2(const uint8_t* password, const size_t password_length,
                            const uint8_t* salt, const size_t salt_length, uint8_t* key,
                            const size_t* key_length)
{
    if (!PKCS5_PBKDF2_HMAC((const char*)password, (const int)password_length, salt,
                           (const int)salt_length, 0, EVP_sha256(), (const int)(*key_length),
                           key)) {
        return SOTER_FAIL;
    }
...
soter_status_t soter_nokdf(const uint8_t* password, const size_t password_length, uint8_t* key,
                           const size_t* key_length)
{
...
soter_status_t soter_withkdf(uint32_t alg, const uint8_t* password, const size_t password_length,
                             const uint8_t* salt, const size_t salt_length, uint8_t* key,
                             size_t* key_length)
{
...
soter_sym_ctx_t* soter_sym_ctx_init(const uint32_t alg, const void* key, const size_t key_length,
                                    const void* salt, const size_t salt_length, const void* iv,
                                    const size_t iv_length, bool encrypt)
{
    ...
    SOTER_IF_FAIL_(soter_withkdf(alg, key, key_length, salt, salt_length, key_, &key_length_) ==
                       SOTER_SUCCESS,
                   soter_sym_encrypt_destroy(ctx));
    if (encrypt) {
        SOTER_IF_FAIL_(EVP_EncryptInit_ex(&(ctx->evp_sym_ctx), evp, NULL, key_, iv),
                       soter_sym_encrypt_destroy(ctx));
    } else {
        SOTER_IF_FAIL_(EVP_DecryptInit_ex(&(ctx->evp_sym_ctx), evp, NULL, key_, iv),
                       soter_sym_encrypt_destroy(ctx));
    }
    return ctx;
}

soter_sym_ctx_t* soter_sym_aead_ctx_init(const uint32_t alg, const void* key,
                                         const size_t key_length, const void* salt,
                                         const size_t salt_length, const void* iv,
                                         const size_t iv_length, bool encrypt)
{
...
soter_status_t soter_sym_ctx_update(soter_sym_ctx_t* ctx, const void* in_data,
                                    const size_t in_data_length, void* out_data,
                                    size_t* out_data_length, bool encrypt)
{
    if (encrypt) {
        SOTER_CHECK(EVP_EncryptUpdate(&(ctx->evp_sym_ctx), out_data, (int*)out_data_length,
                                      (void*)in_data, (int)in_data_length) == 1);
    } else {
        SOTER_CHECK(EVP_DecryptUpdate(&(ctx->evp_sym_ctx), out_data, (int*)out_data_length,
                                      (void*)in_data, (int)in_data_length) == 1);
    }
    return SOTER_SUCCESS;
}

soter_status_t soter_sym_ctx_final(soter_sym_ctx_t* ctx, void* out_data, size_t* out_data_length,
                                   bool encrypt)
{
...
soter_status_t soter_sym_aead_ctx_final(soter_sym_ctx_t* ctx, bool encrypt)
{
    ...
    if (encrypt) {
        SOTER_CHECK(EVP_EncryptFinal_ex(&(ctx->evp_sym_ctx), out_data, (int*)&out_data_length) !=
                        0 &&
                    out_data_length == 0);
    } else {
        SOTER_CHECK(EVP_DecryptFinal_ex(&(ctx->evp_sym_ctx), out_data, (int*)&out_data_length) !=
                        0 &&
                    out_data_length == 0);
    }
    return SOTER_SUCCESS;
...
soter_sym_ctx_t* soter_sym_encrypt_create(const uint32_t alg, const void* key,
                                          const size_t key_length, const void* salt,
                                          const size_t salt_length, const void* iv,
...
soter_status_t soter_sym_encrypt_update(soter_sym_ctx_t* ctx, const void* plain_data,
                                        const size_t plain_data_length, void* cipher_data,
                                        size_t* cipher_data_length)
{
    ...
    return soter_sym_ctx_update(ctx, plain_data, plain_data_length, cipher_data, cipher_data_length,
                                true);
}
...
soter_sym_ctx_t* soter_sym_decrypt_create(const uint32_t alg, const void* key,
                                          const size_t key_length, const void* salt,
                                          const size_t salt_length, const void* iv,
                                          const size_t iv_length)
{
...
soter_status_t soter_sym_decrypt_update(soter_sym_ctx_t* ctx, const void* cipher_data,
                                        const size_t cipher_data_length, void* plain_data,
                                        size_t* plain_data_length)
{
    ...
    return soter_sym_ctx_update(ctx, cipher_data, cipher_data_length, plain_data, plain_data_length,
                                false);
}
...
soter_sym_ctx_t* soter_sym_aead_encrypt_create(const uint32_t alg, const void* key,
                                               const size_t key_length, const void* salt,
                                               const size_t salt_length, const void* iv,
                                               const size_t iv_length)
{
                                            ...
                                            size_t* auth_tag_length)
{
    ...
    SOTER_CHECK(EVP_CIPHER_CTX_ctrl(&(ctx->evp_sym_ctx), EVP_CTRL_GCM_GET_TAG,
                                    SOTER_AES_GCM_AUTH_TAG_LENGTH, auth_tag));
    (*auth_tag_length) = SOTER_AES_GCM_AUTH_TAG_LENGTH;
    return SOTER_SUCCESS;
}
...
soter_sym_ctx_t* soter_sym_aead_decrypt_create(const uint32_t alg, const void* key,
                                               const size_t key_length, const void* salt,
                                               const size_t salt_length, const void* iv,
                                               const size_t iv_length)
{
                                            ...
                                            const size_t auth_tag_length)
{
    ...
    SOTER_IF_FAIL(EVP_CIPHER_CTX_ctrl(&(ctx->evp_sym_ctx), EVP_CTRL_GCM_SET_TAG,
                                      SOTER_AES_GCM_AUTH_TAG_LENGTH, (void*)auth_tag),
