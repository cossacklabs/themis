...
soter_sym_aead_ctx_t* soter_sym_aead_ctx_init(const uint32_t alg,
				    const void* key,
				    const size_t key_length,
				    const void* salt,
				    const size_t salt_length,
  ...
  SOTER_CHECK_PARAM_(key!=NULL);
  SOTER_CHECK_PARAM_(key_length!=0);
  soter_sym_aead_ctx_t* ctx=NULL;
  ctx=malloc(sizeof(soter_sym_aead_ctx_t));
  SOTER_CHECK_MALLOC_(ctx);
  ctx->alg=alg;
  uint8_t key_[SOTER_SYM_MAX_KEY_LENGTH];
  size_t key_length_=(alg&SOTER_SYM_KEY_LENGTH_MASK)/8;
  EVP_CIPHER_CTX_init(&(ctx->evp_sym_ctx));
  SOTER_IF_FAIL_(soter_withkdf(alg,key, key_length, salt, salt_length, key_, &key_length_)==SOTER_SUCCESS, soter_sym_aead_encrypt_destroy(ctx));
  if(encrypt){
    SOTER_IF_FAIL_(EVP_EncryptInit_ex(&(ctx->evp_sym_ctx), algid_to_evp_aead(alg), NULL, key_, iv), soter_sym_aead_encrypt_destroy(ctx));
  } else {
    SOTER_IF_FAIL_(EVP_DecryptInit_ex(&(ctx->evp_sym_ctx), algid_to_evp_aead(alg), NULL, key_, iv), soter_sym_aead_encrypt_destroy(ctx));
  }
  return ctx;
}
...
soter_status_t soter_sym_aead_ctx_final(soter_sym_aead_ctx_t *ctx,bool encrypt){
  uint8_t out_data[16];
...
soter_status_t soter_sym_aead_encrypt_update(soter_sym_aead_ctx_t *ctx, const void* plain_data,  const size_t plain_data_length, void* cipher_data, size_t* cipher_data_length){
  ...
  (*cipher_data_length)=plain_data_length+EVP_CIPHER_CTX_block_size(&(ctx->evp_sym_ctx))-1;
  return soter_sym_ctx_update((soter_sym_ctx_t*)ctx, plain_data, plain_data_length, cipher_data, cipher_data_length, true);
}

soter_status_t soter_sym_aead_encrypt_aad(soter_sym_aead_ctx_t *ctx, const void* plain_data,  const size_t plain_data_length){
  size_t tmp=0;
  return soter_sym_ctx_update((soter_sym_ctx_t*)ctx, plain_data, plain_data_length, NULL, &tmp, true);
}
...
soter_status_t soter_sym_aead_encrypt_destroy(soter_sym_aead_ctx_t *ctx){
  return soter_sym_ctx_destroy((soter_sym_ctx_t*)ctx);
}
...
soter_status_t soter_sym_aead_decrypt_update(soter_sym_aead_ctx_t *ctx, const void* cipher_data,  const size_t cipher_data_length, void* plain_data, size_t* plain_data_length){
  ...
  return soter_sym_ctx_update((soter_sym_ctx_t*)ctx, cipher_data,  cipher_data_length, plain_data, plain_data_length, false);
}

soter_status_t soter_sym_aead_decrypt_aad(soter_sym_aead_ctx_t *ctx, const void* plain_data,  const size_t plain_data_length){
  size_t tmp=0;
  return soter_sym_ctx_update((soter_sym_ctx_t*)ctx, plain_data, plain_data_length, NULL, &tmp, false);
}
