...
#define SOTER_SYM_BLOCK_LENGTH(alg) ((alg&SOTER_SYM_BLOCK_LENGTH_MASK)>>12)
...
soter_sym_ctx_t* soter_sym_ctx_init(const uint32_t alg,
  ...
  if(iv!=NULL && (iv_length<SOTER_SYM_BLOCK_LENGTH(alg))){
    return NULL;
  }
  ...
  if(encrypt){
    ...
    HERMES_IF_FAIL_(EVP_DecryptInit_ex(&(ctx->evp_sym_ctx), algid_to_evp(alg), NULL, key_, iv), soter_sym_encrypt_destroy(ctx));
  }
  return ctx;
}
...
soter_sym_ctx_t* soter_sym_aead_ctx_init(const uint32_t alg,
  ...
  if(encrypt){
    ...
    HERMES_IF_FAIL_(EVP_DecryptInit_ex(&(ctx->evp_sym_ctx), algid_to_evp_aead(alg), NULL, key_, iv), soter_sym_encrypt_destroy(ctx));
  }
  return ctx;
}
...
soter_status_t soter_sym_ctx_update(soter_sym_ctx_t *ctx,
				    ...
				    bool encrypt){
  if(encrypt){
    HERMES_CHECK(EVP_EncryptUpdate(&(ctx->evp_sym_ctx), out_data, (int*)out_data_length, (void*)in_data, in_data_length)==1);
  } else {
    HERMES_CHECK(EVP_DecryptUpdate(&(ctx->evp_sym_ctx), out_data, (int*)out_data_length, (void*)in_data, in_data_length)==1);
  }    
  return HERMES_SUCCESS;
}
...
soter_status_t soter_sym_ctx_final(soter_sym_ctx_t *ctx,
  ...
  if(encrypt){
    ...
    HERMES_CHECK(EVP_DecryptFinal(&(ctx->evp_sym_ctx), out_data, (int*)out_data_length)!=0);
  }    
  return HERMES_SUCCESS;
}
...
soter_status_t soter_sym_encrypt_update(soter_sym_ctx_t *ctx, const void* plain_data,  const size_t plain_data_length, void* cipher_data, size_t* cipher_data_length){
   if((*cipher_data_length)<(plain_data_length+SOTER_SYM_BLOCK_LENGTH(ctx->alg)-1)){
     (*cipher_data_length)=plain_data_length+SOTER_SYM_BLOCK_LENGTH(ctx->alg);
...
soter_status_t soter_sym_encrypt_final(soter_sym_ctx_t *ctx, void* cipher_data, size_t* cipher_data_length){
   if((*cipher_data_length)<SOTER_SYM_BLOCK_LENGTH(ctx->alg)){
     (*cipher_data_length)=SOTER_SYM_BLOCK_LENGTH(ctx->alg);
     return HERMES_BUFFER_TOO_SMALL;
   }
...
soter_status_t soter_sym_decrypt_update(soter_sym_ctx_t *ctx, const void* cipher_data,  const size_t cipher_data_length, void* plain_data, size_t* plain_data_length){
   if((*plain_data_length)<(cipher_data_length+SOTER_SYM_BLOCK_LENGTH(ctx->alg))){
     (*plain_data_length)=cipher_data_length+SOTER_SYM_BLOCK_LENGTH(ctx->alg);
      return HERMES_BUFFER_TOO_SMALL;
   }
...
soter_status_t soter_sym_aead_encrypt_update(soter_sym_ctx_t *ctx, const void* plain_data,  const size_t plain_data_length, void* cipher_data, size_t* cipher_data_length){
   if((*cipher_data_length)<(plain_data_length+SOTER_SYM_BLOCK_LENGTH(ctx->alg)-1)){
     (*cipher_data_length)=plain_data_length+SOTER_SYM_BLOCK_LENGTH(ctx->alg)-1;
      return HERMES_BUFFER_TOO_SMALL;
   }
   return soter_sym_ctx_update(ctx, plain_data, plain_data_length, cipher_data, cipher_data_length, true);
}

soter_status_t soter_sym_aead_encrypt_final(soter_sym_ctx_t *ctx, void* cipher_data, size_t* cipher_data_length, void* auth_tag, size_t* auth_tag_length){
  ...
  HERMES_CHECK(soter_sym_ctx_final(ctx, cipher_data, cipher_data_length, true)==HERMES_SUCCESS);
...
soter_status_t soter_sym_aead_decrypt_final(soter_sym_ctx_t *ctx, void* plain_data, size_t* plain_data_length, const void* auth_tag, const size_t auth_tag_length)
{
  HERMES_CHECK_PARAM(auth_tag!=NULL);
  HERMES_CHECK_PARAM(auth_tag_length>=SOTER_AES_GCM_AUTH_TAG_LENGTH);
  HERMES_CHECK(ctx!=NULL);
  HERMES_IF_FAIL(EVP_CIPHER_CTX_ctrl(&(ctx->evp_sym_ctx), EVP_CTRL_GCM_SET_TAG, SOTER_AES_GCM_AUTH_TAG_LENGTH, (void*)auth_tag), soter_sym_aead_decrypt_destroy(ctx));
  return  soter_sym_ctx_final(ctx, plain_data, plain_data_length, false);
}
