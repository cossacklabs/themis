...
soter_status_t soter_hash_update(soter_hash_ctx_t *hash_ctx, const void *data, size_t length)
{
	...
	if(0 == crypto_generichash_update(&(hash_ctx->md_ctx), data, length)){
		return SOTER_SUCCESS;
	}else{
		return SOTER_FAIL;
	}
...
{
	size_t md_length = crypto_generichash_BYTES;
	if (!hash_ctx){
		return SOTER_INVALID_PARAMETER;
	}
	...
	if (crypto_generichash_final(&(hash_ctx->md_ctx), hash_value, (unsigned int *)&md_length)){
		*hash_length = md_length;
		return SOTER_SUCCESS;
	}else{
		return SOTER_FAIL;
...
soter_hash_ctx_t* soter_hash_create(soter_hash_algo_t algo) //algo parameter not used
{
