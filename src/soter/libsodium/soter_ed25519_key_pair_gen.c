...
struct soter_ed25519_key_pair_gen_type{
    soter_ed25519_priv_key_t priv;
    soter_ed25519_pub_key_t pub;
};
...
soter_status_t soter_ed25519_key_pair_gen_cleanup(soter_ed25519_key_pair_gen_t* ctx){
    if(!ctx){
	return SOTER_INVALID_PARAMETER;
    }
    memset(ctx, 0, sizeof(soter_ed25519_key_pair_gen_t));
    return SOTER_SUCCESS;
}
