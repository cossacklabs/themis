...
class scomparator(object):
    def __init__(self, shared_secret):
        ...
        res = themis.secure_comparator_append_secret(self.comparator_ctx, ctypes.byref(ctypes.create_string_buffer(shared_secret)), len(shared_secret));
    ...
    def proceed_compare(self, control_message):
        ...
        req_size=ctypes.c_int(0);
        res = themis.secure_comparator_proceed_compare(self.comparator_ctx, ctypes.byref(c_message), len(control_message), None, ctypes.byref(req_size));
        if res == THEMIS_CODES.SUCCESS:
            return "";
        if res!=THEMIS_CODES.BUFFER_TOO_SMALL:
            raise exception.themis_exception(res, "secure_comparator_proceed_compare (buffer_length determination) failed");
        req_buffer=ctypes.create_string_buffer(req_size.value);
        res = themis.secure_comparator_proceed_compare(self.comparator_ctx, ctypes.byref(c_message), len(control_message), req_buffer, ctypes.byref(req_size));
